const Engine = Matter.Engine;
const Render = Matter.Render;
const World = Matter.World;
const Bodies = Matter.Bodies;
const Constraint = Matter.Constraint;
const Body = Matter.Body;
const Composites = Matter.Composites;
const Composite = Matter.Composite;

let engine;
let world;
var ball;
var blower;
var blowerMouth;
var button;

function setup() {
  var canvas = createCanvas(500, 500);

  engine = Engine.create();
  world = engine.world;

  ball = new Ball(width / 2 + 120, height / 2 - 80, 80, 80);
  blower = new Blower(width / 2 , height / 2 + 50, 150, 20);
  blowerMouth = new BlowerMouth(width / 2 + 120, height / 2 + 20, 100, 90);
  blowerMouth2 = new BlowerMouth(width / 2 - 170, height / 2 + 20, 100, 100);
  ball2 = new Ball(width/ 2 - 170, height / 2 - 80, 80, 80);
  button = createButton("Click to Blow");
  button.position(width / 2 + 40, height - 100);
  button.class("blowButton");
  button2 = createButton("Click me");
  button2.position(width / 2 - 230, height - 100);
  button2.class("blowButton");

  button.mousePressed(blow);
  button2.mousePressed(blow);

  //buttonPressed(blow);
  
  //button = mousePressed(blow);
  
  //button.mousePressed();

}

function draw() {
  background("violet");
  Engine.update(engine);

  blower.show();
  ball.show();
  ball2.show();
  blowerMouth.show();
  blowerMouth2.show();
}

function blow() {

  Matter.Body.applyForce(ball.body, {x:0, y:0}, {x:0, y:-0.05});

  //Matter.Body.applyForce(ball.body, {x:0, y:0}, {x:0, y:0.05});
  
  //Matter.Body.applyForce(ball.body, {x:0, y:0}, {x:0.05, y:0.05});
  
  //Matter.Body.applyForce(ball.body, {x:0, y:0}, {x:-0.05, y:0});

}

